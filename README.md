...
This project is a digital human that can talk and listen to you. It uses OpenAI's GPT-3 to generate responses, Deepgram
to transcribe the audio, Sarvam AI to generate voice and Rhubarb Lip Sync to generate the lip sync. The tutorial
...
* This service integrates with Sarvam AI TTS and Rhubarb Lip-Sync to generate the following client integration interface, where the exchanged data looks something like this:
```js
[
  {
...
3. **Audio Generation:** The response from GPT is relayed to the Sarvam AI TTS API to generate audio.
...
2. **Speech-to-Text Conversion:** The audio is transmitted to the Deepgram API to convert it into text.
3. **Text Processing:** The converted text is sent to the OpenAI GPT API for further processing.
4. **Audio Generation:** The output from GPT is sent to the Sarvam AI TTS API to produce audio.
...
2. **Deepgram Subscription:** You need to have a subscription with Deepgram for speech-to-text transcription. If you don't have one yet, you can
   sign up [here](https://console.deepgram.com/signup). 
   Get your API key from the [Deepgram Console](https://console.deepgram.com/).
3. **Sarvam AI Subscription:** You need to have a subscription with Sarvam AI. If you don't have one yet, you can
   sign up [here](https://dashboard.sarvam.ai/). 
   Get your API key from the [Sarvam AI Dashboard](https://dashboard.sarvam.ai/).
4. **Rhubarb Lip-Sync:** Download the latest version of Rhubarb Lip-Sync compatible with your operating system from the
...
5. Install `ffmpeg` for  [Mac OS](https://formulae.brew.sh/formula/ffmpeg), [Linux](https://ffmpeg.org/download.html) or [Windows](https://ffmpeg.org/download.html).
...
SARVAM_SPEAKER=Anushka  # Options: Anushka, Manisha, Vidya, Arya, Abhilash, Karun, Hitesh
SARVAM_LANGUAGE=hi-IN   # Language code (e.g., hi-IN for Hindi, en-IN for English)
```
